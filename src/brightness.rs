...
impl Brightness {
    ...
    pub const DIMMEST: Brightness = Brightness::custom(0x01, 0x00);

    /// A dim predefined brightness level
    pub const DIM: Brightness = Brightness::custom(0x02, 0x2F);

    /// A medium predefined brightness level
    pub const NORMAL: Brightness = Brightness::custom(0x04, 0x7F);

    /// A bright predefined brightness level
    pub const BRIGHT: Brightness = Brightness::custom(0x04, 0x9F);

    /// The brightest predefined brightness level
    pub const BRIGHTEST: Brightness = Brightness::custom(0x04, 0xFF);

    /// Create a Brightness object from a precharge period and contrast pair.
    ///
    /// `precharge` sets the `phase 2` argument of the `0xBC Set Pre-Charge Period` command and must
    ...
    /// See section 10.1.17 of the SSD1327 datasheet for more information.
    ///
    /// `contrast` sets the value used in the `0x81 Set Contrast Control` command and must be
    /// between 0 and 255. See section 10.1.7 of the SSD1327 datasheet for more information.
